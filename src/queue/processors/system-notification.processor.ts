...
export class SystemNotificationProcessor extends WorkerHost {
  ...
  async process(job: Job<any>): Promise<any> {
    ...
    this.logger.log(`Processing system notification job ${job.id} for user ${userId}`);
    ...
    try {
      ...
      // Special handling for FORGOT_PASSWORD
      if (type === 'FORGOT_PASSWORD') {
        const { resetToken, frontendUrl } = context;
        if (!resetToken) {
          this.logger.error('Missing resetToken for FORGOT_PASSWORD job');
          return { success: false, message: 'Missing resetToken' };
        }

        await this.emailService.sendForgotPasswordEmail({
          toEmail: user.email,
          name: user.fullName || user.email,
          resetUrl: `${frontendUrl || process.env.APP_URL}/reset-password?token=${resetToken}`,
        });
      ...
      this.logger.log(`System notification job ${job.id} completed successfully`);
      return {
        success: true,
        notificationId: notification.id,
